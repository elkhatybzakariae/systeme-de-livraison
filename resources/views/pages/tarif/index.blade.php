        ...
        <form id="kt_modal_new_tarif_form" method="POST" class="form" action="{{ route('tarif.store') }}">
          ...
          <div class="row">
            ...
            <div class=" col-md-6 col mb-8 fv-row">
              ...
              <input type="number" class="form-control form-control-solid" placeholder="Enter Prix Livraison" id="prixliv" name="prixliv" />
            </div>
            <div class=" col-md-6 col mb-8 fv-row">
              <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                <span class="required">Prix Retour</span>
              </label>
              <input type="number" class="form-control form-control-solid" placeholder="Enter Prix Retour" id="prixret" name="prixret" />
            </div>
            <div class=" col-md-6 col mb-8 fv-row">
              <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                <span class="required">Prix Référence</span>
              </label>
              <input type="number" class="form-control form-control-solid" placeholder="Enter Prix Référence" id="prixref" name="prixref" />
            </div>
            <div class=" col-md-6 col mb-8 fv-row">
              <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                <span class="required">Délai Livraison</span>
              </label>
              <input type="text" class="form-control form-control-solid" placeholder="Enter Délai Livraison" id="delailiv" name="delailiv" />
            </div>
          </div>
      ...
      <tbody class="fw-semibold text-gray-600">
        @foreach ($tarifs as $index=>$tarif)
          <tr>
            ...
            <td class="pe-0">
              <span class="fw-bold" data-kt-ecommerce-product-filter="prixret">{{ $tarif->prixret }}</span>
            </td>
            <td class="pe-0">
              <span class="fw-bold" data-kt-ecommerce-product-filter="prixref">{{ $tarif->prixref }}</span>
            </td>
            <td class="pe-0">
              <span class="fw-bold" data-kt-ecommerce-product-filter="delailiv">{{ $tarif->delailiv }}</span>
            </td>
            <!--begin::Action=-->
            <td class="">
              ...
              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                <div class="menu-item px-3">
                  <a onclick="openModal('{{ $tarif->villeRamassage }}','{{ $tarif->ville }}','{{ $tarif->prixliv }}','{{ $tarif->prixret }}','{{ $tarif->prixref }}','{{ $tarif->delailiv }}','{{ route('tarif.update',$tarif->id_Tar) }}')" data-bs-toggle="modal" data-bs-target="#kt_modal_new_target" class="menu-link px-3">Edit</a>
                </div>
                <div class="menu-item px-3">
