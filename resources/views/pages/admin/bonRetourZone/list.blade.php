      ...
      <tbody class="fw-semibold text-gray-600">
        @foreach ($bons as $index=>$item)
          <tr>
            ...
            <td class="pe-0">
              <span class="fw-bold" data-kt-ecommerce-product-filter="nomcomplet">{{ $item->zone->zonename }}</span>
            </td>
            ...
            <td>
              ...
              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-250px py-4" data-kt-menu="true">
                <div class="menu-item px-3">
                  <a  class="btn" onclick="openModal('{{ $item->id_BRZ }}')"  data-bs-toggle="modal" data-bs-target="#kt_modal_new_target"><i class="fa fa-eye"></i>Details du bon</a>
                </div>
                <div class="menu-item px-3">
                  <a  class="btn" href="{{ route('bon.retour.zone.exportColis',$item->id_BRZ) }}"><i class="far fa-file-excel"></i>Exporter les colis</a>
                </div>
                @if ($item->status != 'recu')
                    <div class="menu-item px-3">
                  <form action="{{route('bon.retour.zone.recu',$item->id_BRZ)}}" method="post">
                    @csrf
                ...
                @endif
                <div class="menu-item px-3">
                  <a  class="btn"  href="{{ route('bon.retour.zone.getPdf',$item->id_BRZ) }}"><i class="far fa-file-pdf"></i>Voir en Pdf</a>
                </div>
                @if ($item->colis->count()==0)
                  <div class="menu-item  text-hover-danger px-3">        
                    <a  href="{{ route('bon.retour.zone.destroy',$item->id_BRZ) }}" class="btn">
